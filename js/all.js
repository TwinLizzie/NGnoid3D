...
(function() {
  var Page, Promise, Text, Time, ZeroFrame, call_after_interval, calling, design_creator, design_editor, editor, last_time, left_menuify, profile_editor, seedbox, top_menuify, uploader, video_lister, video_playing, videobox,
    indexOf = [].indexOf,
...
// Not called within interval, call instantly
  ...
  left_menuify = class left_menuify {
    ...
    render() {
      ...
      $("#item_latest_designs_link").on("click", function() {
        return Page.nav(this.href);
      });
  ...
  top_menuify = class top_menuify {
    ...
    render() {
      var channel_icon, channel_link, design_icon, design_link, logo, main_menu, menu_top, search_bar, search_button, search_icon, search_swap, search_swap_icon, search_wrap, toggle_button, toggle_icon, upload_icon, upload_link;
      menu_top = $("<div></div>");
  ...
  uploader = class uploader {
    ...
    upload_file(files) {
      var file_info, register_upload, time_stamp, upload_done;
  ...
  design_editor = class design_editor {
    ...
    delete_from_data_json(design_url, design_name, cb) {
      var data_inner_path, design_directory;
      ...
      return Page.cmd("fileGet", data_inner_path, (res) => {
        ...
        return Page.cmd("fileWrite", [data_inner_path, Text.fileEncode(data)], cb);
      });
    }

    delete_design(design_url, design_name) {
      var content_inner_path, delete_from_data_json, design_directory;
      ...
      return Page.cmd("wrapperConfirm", ["Are you sure?", "Delete"], () => {
        return delete_from_data_json(design_url, design_name, function(res) {
          if (res === "ok") {
            Page.cmd("sitePublish", {
              "inner_path": content_inner_path
            });
    ...
    save_info(d_name, d_title, d_description, d_image, d_date) {
      ...
      register_info = this.register_info;
      return this.check_content_json((res) => {
        return register_info(d_name, d_title, d_description, d_image, d_date, (res) => {
          return Page.cmd("siteSign", {
            inner_path: "data/users/" + Page.site_info.auth_address + "/content.json"
          }, function(res) {
            return Page.cmd("sitePublish", {
    ...
    render() {
      ...
      Page.cmd("dbQuery", [query], (res) => {
        ...
        if (res.length === 0) {
          ...
          return console.log("user address: " + user_address);
        } else {
          my_row = res[0];
          ...
          if (user_directory === Page.site_info.auth_address) {
            ...
            save_info = this.save_info;
            $("#editor_submit_button").on("click", function(e) {
              save_info(design_name, $("#editor_title").val(), $("#editor_brief").val(), $("#thumbnail_input").val(), design_date_added);
              return e.preventDefault();
            });
            delete_design = this.delete_design;
            return $("#delete_design_button").on("click", function(e) {
  ...
  design_creator = class design_creator {
    constructor() {
      this.convert_base64 = this.convert_base64.bind(this);
      this.check_content_json = this.check_content_json.bind(this);
      this.register_design = this.register_design.bind(this);
      this.create_done = this.create_done.bind(this);
      this.create_design = this.create_design.bind(this);
      this.render = this.render.bind(this);
    }
    ...
    register_design(name, title, description, image_link, date_added, cb) {
      ...
      return Page.cmd("fileGet", [inner_path, false], (res) => {
        ...
        if (res.design === null || res.design === void 0) {
          res.design = {};
        }
        res.design[name] = {
          title: title,
    ...
    create_done(name, title, date_added, user_address) {
      ...
      return console.log("Design creation done! ", name);
    }

    create_design(design_title, design_brief, design_image) {
      var create_done, file_info, new_design_name, register_design, time_stamp;
      time_stamp = Math.floor(new Date() / 1000);
      console.log("Creating new design: " + design_title);
      file_info = this.file_info = {};
      register_design = this.register_design;
      create_done = this.create_done;
      new_design_name = design_title.replace(/\s/g, "_");
      return this.check_content_json((res) => {
        var stamp_design_name;
        stamp_design_name = time_stamp + "-" + new_design_name;
        console.log("loadend", arguments);
        file_info.status = "done";
        return register_design(stamp_design_name, design_title, design_brief, design_image, time_stamp, function(res) {
          return Page.cmd("siteSign", {
            inner_path: "data/users/" + Page.site_info.auth_address + "/content.json"
          }, function(res) {
            return Page.cmd("sitePublish", {
              ...
              return create_done(stamp_design_name, design_title, time_stamp, Page.site_info.auth_address);
            });
          });
        });
      });
    ...
    render() {
      var brief_div, brief_input, brief_label, convert_base64, create_design, design_container, design_editorbox, editor_submit, thumbnail_container, thumbnail_div, thumbnail_image, thumbnail_input, thumbnail_title, thumbnail_upload, thumbnail_upload_label, title_div, title_input, title_label;
      ...
      create_design = this.create_design;
      return $("#editor_submit_button").on("click", function(e) {
        if (Page.site_info.cert_user_id) {
          ...
          Page.cmd("certSelect", [["zeroid.bit"]], (res) => {
            create_design($("#design_name").val(), $("#editor_brief").val(), $("#thumbnail_input").val());
            $("#design_container").html("<div class='spinner'><div class='bounce1'></div></div>");
            return console.log("[NGnoid 3d: Creating design...]");
          });
        }
        return e.preventDefault();
      });
  ...
  videobox = class videobox {
    ...
    display_proxy_warn() {
      if (window.location.origin === "https://portal.ngnoid.tv") {
        return $("#videobox").html("<p style='color: white; margin-left: 10px'>Your model list is empty.</p><p style='color: white; margin-left: 10px'>New to ZeroNet? Check out the PC and mobile client at <a href='https://zeronet.dev'>ZeroNet.dev</a></p><p style='color: white; margin-left: 10px'>If you're on Linux, simply clone my <a href='https://github.com/TwinLizzie/ZeroNet'>Github repository</a> and:</p><p style='color: white; margin-left: 10px'>chmod +x install.sh && ./install.sh</p>");
      }
    }
  ...
  video_lister = class video_lister {
    constructor() {
      ...
      this.linkify_design = this.linkify_design.bind(this);
      ...
      this.print_design_row = this.print_design_row.bind(this);
    ...
    print_design_row(item) {
      var design_brief, design_channel_name, design_date_added, design_description, design_edit_link, design_edit_link_id, design_image, design_info, design_info_id, design_link, design_link_id, design_name, design_row, design_row_id, design_string, design_thumbnail, design_thumbnail_id, design_title, design_user_address, elementExists, full_channel_name, user_info, user_info_id, vimCheckHttp;
      ...
      design_row_id = "row_" + this.counter;
      ...
      if (elementExists === null) {
        ...
        design_thumbnail_id = "thumb_" + this.counter;
        ...
        design_info_id = "info_" + this.counter;
        design_info = $("<div></div>");
        design_info.attr("id", design_info_id);
        design_info.attr("class", "video_info");
        design_link = this.linkify_design(design_string, design_title, "video_link", design_string);
        user_info_id = "user_" + this.counter;
        user_info = $("<a></a>");
        user_info.attr("id", user_info_id);
        user_info.attr("class", "video_brief channel_link");
        ...
        design_edit_link_id = "designedit_" + this.counter;
        ...
        if (Page.site_info.cert_user_id) {
          if (Page.site_info.cert_user_id === full_channel_name) {
            $("#" + design_info_id).append(design_edit_link);
          }
        }
        $("#" + design_link_id).text(design_title);
        $("#" + design_link_id).on("click", function() {
          return Page.nav(this.href);
        });
        $("#" + design_thumbnail_id).on("click", function() {
          return Page.nav(this.href);
        });
        if (Page.site_info.cert_user_id) {
          if (Page.site_info.cert_user_id === full_channel_name) {
            $("#" + design_edit_link_id).on("click", function() {
              return Page.nav(this.href);
            });
          }
        }
        ...
        return Page.cmd("dbQuery", [query_full], (res1) => {
          ...
          if (res1.length > 0) {
            ...
            for (k = l = 0, len = res1.length; l < len; k = ++l) {
              row4 = res1[k];
              if (this.counter < this.max_videos) {
                results.push(this.print_design_row(row4));
              } else {
                results.push(void 0);
              }
            }
            ...
            return $("#video_list").html("<p style='color: white; margin-left: 10px'>No designs available yet. Items should show after page download...</p>");
          }
        });
      } else {
        $("#video_list").show();
    ...
    update() {
      ...
      if (this.order_by === "peer") {
        order_actual = {
          orderby: "peer DESC",
          filter: "bigfile",
          address: "1L7aqQovTqoaARXnaTNPV7csErPMJfX3Dp",
          limit: max_videos
        };
        if (this.query_string !== "") {
          ...
          return query_database(query, file_limit, order_actual, "peer");
        }
            ...
            if (Page.site_info.auth_address) {
              ...
              return Page.cmd("dbQuery", ["SELECT * FROM subscription LEFT JOIN json USING (json_id) WHERE directory='" + Page.site_info.auth_address + "'"], (res0) => {
                ...
                if (i === res0.length) {
                  ...
                  return query_database(query_complete, file_limit, order_actual, "latest");
                }
              });
            }
          }
        }, 1000);
      } else if (this.order_by === "designs") {
        order_actual = "";
        if (this.query_string !== "") {
          query_string_no_space = this.query_string.replace(/\s/g, "%");
  ...
  video_playing = class video_playing {
      ...
      return Page.cmd("wrapperConfirm", ["Unreport?", "Ok"], () => {
        return delete_report_from_data_json(file_uri, function(res) {
          if (res === "ok") {
            ...
            return this_load_report(file_uri);
          }
        });
      });
    }
      ...
      return Page.cmd("wrapperConfirm", ["Unlike?", "Ok"], () => {
        return delete_like_from_data_json(file_uri, function(res) {
          if (res === "ok") {
            ...
            return this_load_likes(file_uri);
          }
        });
      });
    }
      ...
      return Page.cmd("wrapperConfirm", ["Unsubscribe?", "Ok"], () => {
        return delete_sub_from_data_json(user_address, function(res) {
          if (res === "ok") {
            ...
            return this_load_subs(file_uri);
          }
        });
      });
    }
      ...
      return Page.cmd("wrapperConfirm", ["Delete comment?", "Delete"], () => {
        return delete_from_data_json(file_uri, cid, body, date_added, function(res) {
          if (res === "ok") {
            ...
            return this_load_comments(file_uri);
          }
        });
      });
    }
    ...
    register_to_design(design_uri, file_uri, date_added, cb) {
      ...
      return Page.cmd("fileGet", [inner_path, false], (res) => {
        ...
        if (res.design_file === null || res.design_file === void 0) {
          res.design_file = {};
        }
        if (res.design_file[design_uri] === null || res.design_file[design_uri] === void 0) {
          res.design_file[design_uri] = [];
        }
        res.design_file[design_uri].push({
          file_uri: file_uri,
          date_added: date_added
        });
        return Page.cmd("fileWrite", [inner_path, Text.fileEncode(res)], cb);
    ...
    subscribe(file_date_added, file_directory) {
      var file_uri, load_subs, register_subscription;
    ...
    add_to_design(option_val, file_uri) {
      var design_uri, option_val_obj, register_to_design;
      option_val_obj = JSON.parse(option_val);
      design_uri = option_val_obj['date_added'] + "_" + option_val_obj['directory'];
      register_to_design = this.register_to_design;
      return editor.check_content_json((res) => {
        return register_to_design(design_uri, file_uri, Time.timestamp(), (res) => {
          return Page.cmd("siteSign", {
            inner_path: "data/users/" + Page.site_info.auth_address + "/content.json"
          }, function(res) {
            return Page.cmd("sitePublish", {
        ...
        if (res7.length > 0) {
          ...
          return res7.forEach((row1, related_index) => {
            ...
            return Page.cmd("dbQuery", ["SELECT * FROM file LEFT JOIN json USING (json_id) WHERE date_added='" + file_date_added + "' AND directory='" + file_user_address + "'"], (res8) => {
              ...
              return $("#" + video_channel_id).on("click", function() {
                return Page.nav(this.href);
              });
            });
          });
    ...
    load_subs(real_url) {
      var file_url, query, video_date_added, video_user_address;
      //init_url = Page.history_state["url"]
        ...
        if (i === res.length) {
          if (is_subscribed) {
            ...
            return $("#unsubscribe_now_button").on("click", function() {
              if (Page.site_info.cert_user_id) {
                return delete_subscription(real_url, video_user_address);
              } else {
                return Page.cmd("certSelect", [["zeroid.bit"]], (res) => {
            ...
            return $("#subscribe_now_button").on("click", function() {
              if (Page.site_info.cert_user_id) {
                return subscribe(video_date_added, video_user_address);
              } else {
                return Page.cmd("certSelect", [["zeroid.bit"]], (res) => {
                  return subscribe(video_date_added, video_user_address);
                });
    ...
    load_report(real_url) {
      var file_uri, query, video_date_added, video_user_address;
      //init_url = Page.history_state["url"]
      //real_url = init_url.split("Model=")[1]
      video_date_added = real_url.split("_")[0];
      video_user_address = real_url.split("_")[1];
    ...
    render_video(video_path) {
      ...
      stlLight = this.stlLight;
      stlLight.position.set(1, 1, 1);
      this.stlScene.add(stlLight);
      this.stlRenderer = new THREE.WebGLRenderer({
        antialias: true
      });
      stlRenderer = this.stlRenderer;
      ...
      return Page.cmd("dbQuery", [query], (res1) => {
        ...
        return Page.cmd("optionalFileInfo", optional_file_path, (res2) => {
          ...
          if (res1.length > 0) {
            ...
            this.load_likes(file_uri);
            this.load_subs(file_uri);
            this.load_report(file_uri);
            this.load_comments(file_uri);
            add_report = this.add_report;
            ...
            } else if (render_mode === "design") {
              this.load_design_files(design_url);
              $("#collect_button").hide();
              $("#collect_icon_button").hide();
            }
            add_to_design = this.add_to_design;
            return $("#collect_icon_button").on("click", function() {
              var collect_icon_submit, collect_submit, design_select;
              ...
              if (Page.site_info.cert_user_id) {
                Page.cmd("dbQuery", ["SELECT * FROM design LEFT JOIN json USING (json_id) WHERE cert_user_id='" + Page.site_info.cert_user_id + "' ORDER BY date_added DESC LIMIT 50"], (res5) => {
                  if (res5.length > 0) {
                    return res5.forEach((row5, index) => {
                      return $("#design_selector").append($('<option></option>').val(JSON.stringify(row5)).text(row5.title));
                    });
                  } else {
                    console.log("Res5 is 0");
                    return $("#design_selector").append($('<option></option>').val("bundles_none").text("No bundles yet"));
                  }
                });
              } else {
                Page.cmd("certSelect", [["zeroid.bit"]], (resExec) => {
                  return Page.cmd("dbQuery", ["SELECT * FROM design LEFT JOIN json USING (json_id) WHERE cert_user_id='" + Page.site_info.cert_user_id + "' ORDER BY date_added DESC LIMIT 50"], (res5) => {
                    if (res5.length > 0) {
                      return res5.forEach((row5, index) => {
                        return $("#design_selector").append($('<option></option>').val(JSON.stringify(row5)).text(row5.title));
                      });
              ...
              return $("#add_2_design").on("click", function() {
                var design_selector_value, parsed_design_selector_value, proper_design_uri;
                design_selector_value = $("#design_selector :selected").val();
                if (design_selector_value === "bundles_none") {
                  return Page.cmd("wrapperNotification", ["info", "Error: No bundles yet! Try creating one first..."]);
                } else {
                  ...
                  return Page.cmd("dbQuery", ["SELECT * FROM design_file LEFT JOIN json USING (json_id) WHERE file_uri='" + file_uri + "' AND design_uri='" + proper_design_uri + "'"], (res9) => {
                    if (res9.length === 0) {
                      add_to_design(design_selector_value, file_uri);
                      $("#additional_info").hide();
                      $("#player_stats").show();
                      $("#collect_button").remove();
                      return $("#collect_icon_button").remove();
                    } else {
                      return Page.cmd("wrapperNotification", ["info", "Error: model already exists in bundle!"]);
                    }
                  });
                }
              });
    ...
    render() {
      var comment_actual, comment_div, design_url, file_uri_index, file_uri_num, init_url, real_url, related_column, related_text, render_player, video_box, video_column, video_info, video_player;
      ...
      render_player = this.render_player;
      if (init_url.indexOf("Model=") > -1) {
        real_url = init_url.split("Model=")[1];
        render_player(real_url, "model");
        return console.log("Rendering model view: " + real_url);
        ...
        return Page.cmd("dbQuery", ["SELECT * FROM design_file LEFT JOIN json USING (json_id) WHERE design_uri='" + design_url + "' ORDER BY date_added ASC LIMIT 50"], (res6) => {
          var first_item;
          if (res6.length > 0) {
            ...
            $("#video_box").html("");
            return $("#video_box").html("<p style='color: white; margin-left: 10px'>Error: This design is empty!</p><p style='color: white; margin-left: 10px'>If you're sure it's not, try:</p><p style='color: white; margin-left: 10px'>1. Clearing your cache</p><p style='color: white; margin-left: 10px'>2. Waiting for ZeroNet to fully download the site.</p>");
          }
        });
      }
    }
  ...
  Page = class Page extends ZeroFrame {
    constructor() {
      var already_rendered, first_time;
      super();
      this.render = this.render.bind(this);
    ...
    render() {
      ...
      if (base.href.indexOf("?") === -1) {
        ...
        if (base.href.indexOf("Model") > -1) {
          ...
          this.route(url, "design_editor");
          this.state = {};
          this.state.page = "design_editor";
        } else if (base.href.indexOf("DesignCreate") > -1) {
          this.route(url, "design_create");
          this.state = {};
          this.state.page = "design_create";
          ...
          this.route(url, "design_user");
          this.state = {};
          this.state.page = "design_user";
        } else if (base.href.indexOf("DesignView") > -1) {
          this.route(url, "design_view");
          this.state = {};
          this.state.page = "design_view";
          ...
          this.route(url, "designs");
          this.state = {};
          this.state.page = "designs";
    ...
    project_this(mode) {
      ...
      } else if (mode === "design_view") {
        return video_playing.render();
      } else if (mode === "upload") {
        return uploader.render();
      } else if (mode === "editor") {
        return editor.render();
      } else if (mode === "design_editor") {
        return design_editor.render();
      } else if (mode === "design_create") {
        return design_creator.render();
    ...
    route(query) {
      ...
      if (query.indexOf("Model") > -1) {
        return this.project_this("model");
        ...
        return this.project_this("design_editor");
      } else if (query.indexOf("DesignCreate") > -1) {
        return this.project_this("design_create");
        ...
        return this.project_this("designs");
      } else if (query.indexOf("DesignUser") > -1) {
        return this.project_this("design_user");
      } else if (query.indexOf("DesignView") > -1) {
        return this.project_this("design_view");
