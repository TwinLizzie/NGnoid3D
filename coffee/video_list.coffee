...
class video_lister
  ...
  linkify: (to, display, tag_class = "", tag_id = "", tag_style = "") =>
    link = "<a id='"+tag_id+"' href='?Model="+to+"'"
  ...
  print_design_row: (item) =>
    design_name = item.name
    design_title = item.title
    design_brief = item.description
    design_image = item.image_link
    design_date_added = item.date_added
    design_user_address = item.directory
    full_channel_name = item.cert_user_id
    design_channel_name = item.cert_user_id.split("@")[0]

    design_string = design_date_added + "_" + design_user_address
    design_row_id = "row_" + @counter
    design_link_id = design_string

    elementExists = document.getElementById(design_row_id)
    if elementExists is null

      design_row = $("<div></div>")
      design_row.attr "id", design_row_id
      design_row.attr "class", "video_row"

      design_thumbnail_id = "thumb_" + @counter
      ...
      design_info = $("<div></div>")
      design_info.attr "id", design_info_id
      design_info.attr "class", "video_info"

      design_link = @linkify_design(design_string, design_title, "video_link", design_string)
      ...
      $("#" + design_row_id).append design_info
      $("#" + design_info_id).append design_link
      $("#" + design_info_id).append user_info
  ...
  update: =>
    ...
    if @order_by is "peer"
      order_actual = {orderby: "peer DESC", filter: "bigfile", address: "1L7aqQovTqoaARXnaTNPV7csErPMJfX3Dp", limit: max_videos}
      if @query_string != ""
