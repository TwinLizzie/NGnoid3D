...
class uploader
    ...
    if files.size > 75 * 1024 * 1024
      if window.location.origin is "https://portal.ngnoid.tv"
        ...
        if files.size > 200 * 1024 * 1024
          Page.cmd("wrapperNotification", ["info", "Maximum file size on this site: 200MB"])
          $("#uploader_title").html "<span>Error!</span>" 
          return false
    if files.size < 0.001 * 1024 * 1024
      Page.cmd("wrapperNotification", ["info", "Minimum file size: 1kb"])
      $("#uploader_title").html "<span>Error!</span>" 
      return false
    if files.name.split(".").slice(-1)[0] != "stl"
      Page.cmd("wrapperNotification", ["info", "Only stl's allowed on this site!"])
      $("#uploader_title").html "<span>Error!</span>" 
      debugger
      return false
  ...
  render: =>
    ...
    uploader_title.text "Upload your model here!"
